...
export default function FileUpload({ id, name, accept = 'image/*', required = false, onFileSelect }: FileUploadProps) {
  ...
  return (
    <label htmlFor={id} className={`flex flex-col items-center justify-center w-full h-36 rounded-xl border-2 border-dashed cursor-pointer transition ${fileName ? 'border-teal-500/40 bg-teal-500/5' : 'border-slate-700 bg-slate-800/50 hover:border-slate-600 hover:bg-slate-800'}`}>
      {preview ? (
